...
export class SubBarItem extends React.Component {
  constructor(props) {
    ...
    // Refrencing the subItems parent to use it in predicate logical operator
    // for calculating offsets instead of event.target.parentNode wich doesn't
    // return a stable value
    this.subBarTab = React.createRef();
  }
  ...
  render() {
    const { index, render, hideSubs, style } = this.props;
    return (
      <div
        id={"sub-item-" + index}
        ref={this.subBarTab}
        onMouseOut={event => {
          ...
          // --Used the ref here for offsets calculations
          const predicate =
            event.pageY >=
              this.subBarTab.current.parentNode.offsetTop +
                this.subBarTab.current.parentNode.offsetHeight ||
            event.pageY <= this.subBarTab.current.parentNode.offsetTop ||
            event.pageX <= this.subBarTab.current.parentNode.offsetLeft ||
            event.pageX >=
              this.subBarTab.current.parentNode.offsetLeft +
                this.subBarTab.current.parentNode.offsetWidth;
          if (predicate) {
            hideSubs();
          }
        }}
...
const constructArrayIsActiveItem = (
  elements,
  keyItemValue,
  arrayisactiveitem
) => {
...
const checkBarPropsWarningsAndErrors = props => {
  ...
  if (
    ...
    console.warn(
      Tag +
        " The Width of Vertical Bar is missing (40px is the default value) !"
    );
  }
  if (
    props.type !== undefined &&
    ...
    console.warn(
      Tag +
        " The Height of Horizontal Bar is missing (40px is the default value) !"
    );
  }
  if (props.theme === undefined) {
    console.log(
      Tag +
        " For more stylish look, Note that you can provide the bar with themes check this link => "
    );
  }
};
...
const constructGlobalStyle = (
  ...
  changedGlobalStyle["bar"] = {
    ...
    boxShadow:
      elementsEffect.innerShadowBarValue +
      " " +
  ...
  if (barType === "horizontal") {
    paddingBarItem =
      elementsSize.verticalSpacingInnerBarSize +
      " " +
      elementsSize.horizontalSpacingInnerBarItemSize;
  } else if (barType === "vertical") {
    paddingBarItem =
      elementsSize.verticalSpacingInnerBarItemSize +
      " " +
  ...
  let commonSubBarItemStyle = {
    ...
    boxShadow:
      elementsEffect.innerShadowSubBarItemValue +
      " " +
      colorPalette.innerShadowSubBarItemColor,
  ...
  if (themeType !== undefined && themeType.includes("boxes")) {
    changedGlobalStyle.barItem.active["boxShadow"] =
      elementsEffect.innerShadowBarItemValue +
      " " +
      colorPalette.innerShadowBarItemColor;
  }
  if (
    themeType !== undefined &&
    themeType.includes("underlined") &&
    themeType.includes("boxes")
  ) {
    if (barType === "horizontal") {
      changedGlobalStyle.barItem.active["borderBottom"] =
        elementsSize.bottomBorderBarItem + " " + colorPalette.underlineBoxColor;
    } else if (barType === "vertical") {
      changedGlobalStyle.barItem.active["borderRight"] =
        elementsSize.rightBorderBarItem + " " + colorPalette.underlineBoxColor;
    }
